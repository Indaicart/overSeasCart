  ...
  const fetchChildFees = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/api/parent/child/${childId}/fees`,
        { headers: { 'Authorization': `Bearer ${token}` } }
      );
      const data = await response.json();
      if (data.success) {
        setChildInfo(data.data.child);
        setFeeData(data.data.fees);
      } else {
    ...
    <div className="p-6">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900 flex items-center">
          ...
          {childInfo?.name}'s Fees
        </h1>
        <p className="text-gray-600 mt-2">
          {childInfo?.class_name} • Roll No: {childInfo?.roll_number}
        </p>
      </div>
        ...
        <div className="bg-yellow-50 border-l-4 border-yellow-400 p-6 mb-8 rounded-lg">
          <div className="flex items-start justify-between">
            <div className="flex items-start flex-1">
              ...
              <div>
                <h3 className="text-lg font-semibold text-yellow-900 mb-2">Payment Due</h3>
                <p className="text-yellow-700 mb-3">
                  Term 2 fee payment of ₹{pendingAmount.toLocaleString()} is due on Oct 15, 2024
                </p>
                <button
